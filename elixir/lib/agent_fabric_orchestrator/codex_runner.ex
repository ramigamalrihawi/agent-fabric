defmodule AgentFabricOrchestrator.CodexRunner do
  ...
  def init(opts) do
    ...
    state =
      case FabricClient.register(socket_path) do
        {:ok, session} ->
    ...
    case apply(provider_mod, :launch, [command, workspace]) do
      {:ok, provider_pid} ->
        ...
        {:ok, %{state | status: :running, provider_pid: provider_pid, heartbeat_timer: timer}}
  ...
  @impl true
  def handle_info({_ref, _result} = msg, state) do
    # Task.async sends {ref, result} on completion. Consume silently;
    # the task's :DOWN handler already manages the lifecycle.
  ...
  defp ensure_fabric_worker_run(state) do
    input = %{
      ...
      metadata: %{runnerId: state.id, launchSource: "agent_fabric_elixir_orchestrator"}
    }
